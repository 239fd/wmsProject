...
- `sql-scripts/` — Postgres init scripts mounted into the four DB containers via `docker-compose.yml` (`userDB.sql`, `organizationDB.sql`, `productDB.sql`, `warehouseDB.sql`). They run **once** on first boot of an empty volume. **Flyway was removed 2026-05-17** — it was never actually wired, the `db/migration/V*.sql` directories no longer exist. After first boot, schema changes go directly into `sql-scripts/<service>DB.sql` + the matching JPA entity field, and the DB volume has to be recreated (`cleanup-docker.ps1` + `deploy-docker.ps1`). Hibernate is `validate` everywhere, so the SQL must match the JPA entities. There is a near-duplicate `backend/SSOService/scripts/user-db.sql` used by SSO's standalone `docker-compose.yaml` — keep both in sync.
- `poyasn.pdf` + `User-flow {работник,бухгалтер,директор}.pdf` at the umbrella root — **the source of truth for what each service should do**. Prior cycles also referenced `Требования к *Service.txt` (five Russian requirements docs) and `Спецификация_требований_разработчика_SRS.pdf`; those files are **no longer present in the repo** (verified 2026-05-24) — treat any remaining mention as historical. `FLOWS.md` is stale (last touched 2026-05-06, all "open" items closed) — don't trust it.
...
- Roles: `WORKER`, `ACCOUNTANT`, `DIRECTOR` (RBAC enforced via `SecurityFilterChain` and per-controller `X-User-Role` header checks downstream of the gateway). The user has explicitly fixed this set — don't add a fourth role. **Display labels (2026-05-25, user-facing only)**: WORKER → «Кладовщик», ACCOUNTANT → «Бухгалтер», DIRECTOR → «Заведующий» — applied in client UI, emails (`organization-service/EmailService.translateRole`), document signatories (`document-service/util/RoleToPosition` → «Заведующий складом»). Enum values, routing key `user.director.deleted`, URL `/register/director` and code identifiers remain English/legacy.
...
- OAuth providers (Yandex, Google) are pre-configured. On HEAD, `SSOService/src/main/resources/application.properties` reads them through `${YANDEX_OAUTH_CLIENT_ID:}` / `${GOOGLE_OAUTH_CLIENT_SECRET:}` etc. with empty defaults — no secret is committed in the current code. **But** a coding-time leak landed in git history after a bypass push-protection on 2026-05-22; until that history is rewritten or rotated in the Google/Yandex consoles, treat any past Git ref as sensitive. SMTP password is the same story (`spring.mail.password=${MAIL_PASSWORD:}`) — note that `req.md` in this repo still contains an active Gmail app-password and must be cleaned up.
...
| 9000 / 9001                      | MinIO S3 API / MinIO console (bucket `wms-documents`, creds `wmsadmin/wmsadmin12345`) |
